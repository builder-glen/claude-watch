...
설정은 **화면에 있는 요소를 그대로** 나열합니다. 보는 것과 설정 항목이 1:1 이라
"이걸 끄면 뭐가 사라지지?"를 따로 상상하지 않아도 됩니다.

| 묶음 | 항목 |
|---|---|
| 상단바 | 주고받은 대화 수 · 서브에이전트 수 · 쓴 토큰/비용 · 바뀐 코드 줄 |
| 탭 | 요약 · 구조 · 변경 · 대화 |
| 요약 탭 | AI 요약 · 스킬 · 도구 호출 통계 · 서브에이전트 · 모델 · 의사결정 · 산출물 |
| 대화 탭 | 도구 실행 로그 · 생각 과정 · 시스템 메시지 · 모델 표시 · 토큰 표시 |
| 파일 | 민감 정보 마스킹 · 경량으로 내보내기 |
...
|---|---|
| 전부 포함 | 1.45 MB |
| 도구 본문·토큰·모델·에이전트 뺌 | 0.79 MB |
| 대화 탭 자체를 뺌(요약·구조·변경만) | 0.42 MB |
...
데이터를 실제로 지울지는 **그 데이터를 쓰는 요소가 전부 꺼졌는가**로 정합니다.
예를 들어 토큰 데이터는 상단바·대화 탭·요약 탭 세 곳이 모두 꺼져야 지웁니다.
...
**첫 실행은 통째로 복사하는 일이라 디스크가 두 배가 됩니다.** 그래서 처음 한 번만,
보관할 양이 **500MB 를 넘으면** 물어봅니다.

- 터미널에서 직접 띄웠으면 그 자리에서 선택합니다
- `statusline.sh` 가 자동 기동한 서버는 물어볼 수 없으므로 **보관을 미루고 안내만** 남깁니다.
  뷰어 왼쪽 목록에 뜨는 `지금 보관 시작` 을 누르면 그때 복사합니다
- 500MB 미만이면 묻지 않고 그냥 합니다
- 임계값은 `CW_ARCHIVE_LIMIT`(바이트) 로 바꿀 수 있습니다
