...
| `GET\|POST /api/export-config` | 내보내기 기본 설정 읽기·저장 |
...
내보내기 설정은 한 번 정해 `~/.claude-watch/export-config.json` 에 저장되고,
이후 내보내기는 그걸 참조합니다. 내보낼 때마다 체크박스를 다시 고르지 않습니다.

| 프리셋 | 크기 | 무엇이 들어가나 |
|---|---|---|
| 나중에 내가 볼 용도 | 2.6 MB | 전부. 단 뷰어가 안 그리는 6000자 뒤는 잘라낸다(화면상 차이 없음) |
| 개발자 동료에게 | 1.4 MB | 경량 + 마스킹. 시스템 메시지만 뺌 |
| 비개발자에게 | 0.78 MB | 위에 더해 도구 결과 본문·토큰/비용·모델·서브에이전트를 뺌 |

**"뺀다"는 화면에서 가리는 게 아니라 파일에서 실제로 지우는 것입니다.** CSS 로만 가리면
받는 사람이 소스 보기로 전부 볼 수 있어 숨겼다는 기대와 어긋납니다.
도구는 이름과 성공 여부를 남기고 결과 본문만 뺍니다 — 무엇을 했는지는 남아야 하니까요.
