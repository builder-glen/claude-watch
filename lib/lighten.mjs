...
export const DEFAULT_EXPORT_CONFIG = {
  ...
  mask: true,
  light: true,
  hide: { tools: false, cost: false, model: false, sys: false, agents: false },
};

// 프리셋 — 설정창에서 고르면 hide/mask/light 가 이 값으로 채워진다.
export const PRESETS = {
  full:   { label: "나중에 내가 볼 용도", mask: false, light: false,
            hide: { tools: false, cost: false, model: false, sys: false, agents: false } },
  dev:    { label: "개발자 동료에게",     mask: true,  light: true,
            hide: { tools: false, cost: false, model: false, sys: true,  agents: false } },
  reader: { label: "비개발자에게",        mask: true,  light: true,
            hide: { tools: true,  cost: true,  model: true, sys: true,  agents: true } },
};

export function applyHide(payload, hide = {}, counts = {}) {
  if (!hide || !Object.values(hide).some(Boolean)) return payload;
  const events = [];
  for (const e of payload.events) {
    if (hide.sys && (e.kind === "system_note" || e.kind === "skill_note")) { counts.sysDropped = (counts.sysDropped || 0) + 1; continue; }
    let x = e;
    if (e.kind === "tool_use" && hide.tools) {
      // 도구 자체를 지우면 "무엇을 했는지"가 사라진다. 이름·성공여부만 남기고 본문만 뺀다.
      const { result, input, ...rest } = e;
      x = { ...rest, result: result == null ? null : "", hidden: true };
      counts.toolsStripped = (counts.toolsStripped || 0) + 1;
    }
    if (hide.cost && x.usage) { const { usage, ...r } = x; x = r; counts.costDropped = (counts.costDropped || 0) + 1; }
    if (hide.model && x.model) { const { model, ...r } = x; x = r; }
    events.push(x);
  }
  const agg = { ...payload.agg };
  if (hide.agents) agg.agents = [];
  if (hide.cost) agg.models = { ...(agg.models || {}), list: [] };
  return { ...payload, events, agg };
}
