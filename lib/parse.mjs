...
function userKind(text) {
  return SYS_NOTE_RE.test(String(text || "")) ? "system_note" : "user_text";
}
...
export function parseTranscript(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    if (type === "user" && typeof content === "string") {
      raw.push({ kind: userKind(content), text: content, ts });
      continue;
    }
    ...
    if (type === "user") {
      ...
      for (const c of content) {
        ...
        if (c.type === "text") text += (text ? "\n" : "") + (c.text || "");
          ...
          resultsById[c.tool_use_id] = { text: flattenResult(c.content), isError: c.is_error === true };
      }
      if (text || imgs) raw.push({ kind: userKind(text), text, ts, images: imgs });
      continue;
    }
    ...
    for (const c of content) {
      ...
      switch (c.type) {
        case "text":
          raw.push({ kind: "assistant_text", text: c.text || "", ts });
          break;
        case "thinking":
          if ((c.thinking || "").trim())
          ...
          break;
        case "tool_use":
          raw.push({ kind: "tool_use", id: c.id, name: c.name, input: c.input || {}, ts });
          break;
        default:
          break; // 모르는 블록은 버림
      }
  ...
  for (const e of raw) {
    ...
    } else if (e.kind === "user_text" && isImgMeta(e.text)) {
