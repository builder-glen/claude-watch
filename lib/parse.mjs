...
export function parseTranscript(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    if (type === "user" && typeof content === "string") {
      raw.push({ kind: userKind(content), text: content, ts, m: ++anchor });
      continue;
    }
    ...
    if (type === "user") {
      ...
      if (text || imgs) raw.push({ kind: userKind(text), text, ts, images: imgs, m: ++anchor });
      continue;
    }
...
export function extractSkills(events) {
  ...
  for (const e of events) {
    ...
    const name = skillNameOf(e.text) || "(이름 불명)";
