...
export function parseTranscript(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    if (!o || typeof o !== "object") continue;   // `null` 한 줄이 세션 전체를 500 으로 만들었다
...
export function extractDecisions(events) {
  ...
  for (const e of events) {
    ...
    while ((m = re.exec(res))) chosen[m[1]] = m[2];
    // 두 번째 형식 — 일부만 답하고 "다시 물어봐 달라"고 한 경우. 도구는 거절로 기록되지만 고른 답은 남는다:
    //   - "질문"\n  Answer: 고른 값        /   - "질문"\n  (No answer provided)
    // 이 형식을 못 읽어 실제로 고른 답이 "선택 없음"으로 보였다(세션 1bc69a4b: 7건 중 2건).
    const skipped = new Set();
    const re2 = /-\s*"([^"\n]+)"\s*\n\s*(?:Answer:\s*(.+)|\(No answer provided\))/g;
    while ((m = re2.exec(res))) { if (m[2]) chosen[m[1]] = m[2].trim(); else skipped.add(m[1]); }
    for (const q of (e.input?.questions || [])) {
      ...
      out.push({ header: q.header || "", question: q.question || "", chosen: chosen[q.question] || "", m: e.m,
        ...(skipped.has(q.question) ? { skipped: true } : {}) });
    }
  }
  return out;
}
...
export function summarizeUsage(text) {
  ...
  // ctx = 마지막 턴이 실제로 모델에 밀어넣은 맥락 크기. total(누적)과는 전혀 다른 수다.
  // 이어받기 비용은 누적이 아니라 이 값이 매 턴 재전송되는 것이라 따로 센다.
  // 서브에이전트(isSidechain) 턴은 본 대화의 맥락이 아니므로 제외한다.
  let ctx = 0;
  // Claude Code 는 응답 하나(message.id)를 내용 블록마다 한 줄씩 나눠 쓰면서 같은 usage 를 반복 기록한다.
  // 줄마다 더하면 부풀려진다 — 실측(세션 1bc69a4b): usage 줄 262개 · 고유 메시지 135개, 중복 76건은 값이
  // 전부 동일. 줄 합 52,639,081 vs 메시지 합 27,281,561(1.93배). modelStats 는 이미 메시지 단위로 세고 있어
  // 같은 화면에서 상단 52.6M / 요약 탭 27.3M 로 어긋났었다.
  const seen = new Set();
  for (const line of text.split("\n")) {
    const t = line.trim();
    if (!t) continue;
    ...
    if (mid) { if (seen.has(mid)) continue; seen.add(mid); }   // id 없는 구버전 줄은 그대로 센다
    ...
    if (o.isSidechain !== true) ctx = i + cw + cr + out;
    const p = priceFor(o.message.model);
    cost += i * p.in + out * p.out + cw * p.cw + cr * p.cr;
  }
  return { input, output, cacheCreate, cacheRead, total: input + output + cacheCreate + cacheRead, ctx, cost };
}
